...
// get how many suitcases have already been revealed
int n = s.nextLine();

//get the position of revealed suitcases from 1 - n and set value of the array at that position to zero
for (int i =0;i<=n;i++){
    int x = s.nextInt();
    a[i] = 0;
}
//add the value of everything in the array
int sum = 0;
for (int i=0;i<=n;i++){
    sum += a[i];
}
